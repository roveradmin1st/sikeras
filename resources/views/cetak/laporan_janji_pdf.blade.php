...
<head>
    <meta charset="utf-8">
    <title>Laporan Janji Iman - {{ $startDate }} sd {{ $endDate }}</title>
    <style>
...
<body>
    ...
    <div class="title">
        LAPORAN REKAPITULASI JANJI IMAN
    </div>

    <div class="info">
        <strong>Periode Setoran:</strong> {{ date('d/m/Y', strtotime($startDate)) }} s/d {{ date('d/m/Y', strtotime($endDate)) }}<br>
    ...
    <table>
        ...
        <tbody>
            ...
            @empty
            <tr>
                <td colspan="5" class="text-center">Tidak ada data setoran janji iman pada periode ini.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="summary">
        <table>
            <tr>
                <th><strong>Total Setoran Terkumpul</strong></th>
                <td><strong>Rp. {{ number_format($totalTerkumpul, 0, ',', '.') }}</strong></td>
            </tr>
        </table>
    </div>
    ...
    <div class="signature">
        <div class="signature-box" style="float: left;">
            <p>Dibuat Oleh,<br><strong>Bendahara Pembangunan</strong></p>
            @php
                $bendahara = \App\Models\User::where('role', 'bendahara_pembangunan')->first();
