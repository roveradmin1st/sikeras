...
@section('title', 'Laporan Persembahan - SIKER')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white/65 backdrop-blur-md p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div>
            <h1 class="text-xl font-bold text-slate-800 tracking-tight">Laporan Persembahan Mingguan</h1>
            <p class="text-xs text-slate-500 mt-0.5 font-medium">Rekapitulasi total persembahan mingguan berdasarkan rentang waktu.</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('kas.laporan.persembahan.cetak', ['church_slug' => request()->route('church_slug'), 'start_date' => $startDate, 'end_date' => $endDate]) }}" 
    ...
    <div class="bg-white rounded-xl border border-slate-150 shadow-sm p-6">
        <form method="GET" action="{{ route('kas.laporan.persembahan', ['church_slug' => request()->route('church_slug')]) }}" class="flex flex-col sm:flex-row items-end gap-4">
            <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
        <div class="bg-white rounded-xl p-5 border border-slate-100 shadow-sm flex items-center justify-between">
            <div>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Persembahan</p>
                <h3 class="text-2xl font-bold text-emerald-600 mt-1">Rp. {{ number_format($totalPersembahan, 0, ',', '.') }}</h3>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-xs text-slate-600">
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-slate-400">Tidak ada data persembahan pada periode ini.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
